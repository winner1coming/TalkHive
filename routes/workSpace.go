...
func SetupWorkspaceRoutes(router *gin.Engine) *gin.RouterGroup {
	...
	r.PUT("/code/edit", workSpace.EditCode)      // 编辑代码文件√
	...
	r.DELETE("/recycle/delete-permanent", workSpace.DeletePermanently) // 永久删除√
	...
	r.PUT("/ddl/complete", workSpace.MarkDDLComplete)  // 勾选完成事件√
	r.DELETE("/ddl/delete", workSpace.DeleteDDL)       // 删除DDL事件√
